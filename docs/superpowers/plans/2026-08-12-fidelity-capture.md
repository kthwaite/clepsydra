...
- **The HTTP body limit is derived, not equal.** `max_request_size_mb` is a *decoded content* budget; the transport carries both the base64 resources and the remaining request content. Setting `DefaultBodyLimit` to the same number makes the decoded check unreachable — the wire limit always fires first, and the reader gets a bare `413` naming nothing, which the Risks section says will read as a bug. The transport limit is therefore `2 × decoded budget + 1 MiB` of envelope headroom, calculated with saturating arithmetic, and the hardcoded router fallbacks match that derivation.
