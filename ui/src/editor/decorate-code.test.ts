...
describe("decorateCode", () => {
  it("returns [] while the grammar bundle is still loading", () => {
    ...
    expect(pending([node, [0]])).toEqual([]);
  });

  it("returns [] for non-code-block nodes", () => {
    const para = { type: "paragraph" as const, children: [{ text: "hi" }] };
    expect(decorateCode([para, [0]])).toEqual([]);
  });
  ...
  it("returns [] for a code-block with no language", () => {
    ...
    expect(decorateCode([node, [0]])).toEqual([]);
  });
  ...
  it("emits token ranges into child text 0 with correct offsets", () => {
    const node = codeBlock("javascript", "const x = 1;");
    const ranges = decorateCode([node, [3]]);
