...
capture returns
`operation: "created"`. A later submission with the same identity and exact
earlier prefix appends only the new suffix and returns
`operation: "appended"`; an identical resubmission returns
`operation: "unchanged"`. The result also reports `path`, `page_id`,
