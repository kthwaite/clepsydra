...
The responsive web application and desktop browser client use the same tailnet
HTTPS origin. Start a TLS server when testing from a browser over the tailnet:
...
in the app data dir. See [configuration.md](/docs/configuration) for details.
For tailnet access, use a certificate covering the browser's tailnet hostname
and ensure the browser trusts its issuing CA.
...
[lsp.md](/docs/lsp) for editor setup.
