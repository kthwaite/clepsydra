...
use clep_api::vault::new_note::{NewNoteError, build_note_path};
use clep_api::vault::page::{PageMeta, write_page_content};
...
mod tests {
    ...
    fn new_note_filename_is_canonical() {
        ...
        assert!(
            clep_api::vault::path::is_canonical_page_filename(fname),
            "filename was: {fname}"
        );
        assert!(created.vault_path.as_str().starts_with("notes/"));
    }
