...
use crate::derivers::cite_key::CiteKeyDeriver;
use crate::derivers::links::LinkDeriver;
use crate::derivers::properties::PropertyDeriver;
use crate::derivers::tags::TagDeriver;
...
impl VaultIndex {
    ...
    pub fn reference_issues(
        ...
        crate::reference_issues::project(&self.conn, filter)
    }
    ...
    pub fn build(&mut self, vault: &Vault) -> Result<BuildStats, IndexError> {
        ...
        tx.commit()?;
        let rubbish_store = RubbishStore::for_vault(vault.root());
    ...
    pub fn index_page(
        ...
        let (kind, kind_inferred) =
            clep_vault::kind::resolve(page.vault_path.as_str(), page.meta.kind);
...
pub fn find_body_start(content: &str) -> usize {
    clep_vault::page::body_offset(content)
}
...
fn upsert_indexed_page(
    ...
    let (kind, kind_inferred) = clep_vault::kind::resolve(pf.vault_path.as_str(), pf.meta.kind);
    let kind_str = kind.as_str();
...
mod rubbish_catalog_tests {
    ...
    use clep_vault::rubbish::{RubbishListEntry, RubbishManifest};
