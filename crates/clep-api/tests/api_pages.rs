...
async fn computed_tag_cutover_create_omits_redundant_stored_tag() {
    ...
    let (stored_meta, _) = clep_api::vault::page::parse_frontmatter(&stored).unwrap();
...
untouched body\n";
    ...
    let (rewritten_meta, rewritten_body) =
        clep_api::vault::page::parse_frontmatter(&rewritten).unwrap();
...
async fn computed_tags_cannot_be_removed_by_replacing_editable_tags() {
    ...
    let (meta, _) = clep_api::vault::page::parse_frontmatter(&stored).unwrap();
