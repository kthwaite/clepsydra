...
use clep_api::api::{AppState, api_router};
...
fn setup_server() -> (TestServer, TempDir) {
    ...
    let purge_hooks: Arc<Vec<Box<dyn clep_api::vault::hooks::RubbishPurgeHook>>> =
        Arc::new(vec![Box::new(
            clep_api::vault::archive_hook::ArchiveDeleteHook {
                cas: Arc::clone(&cas),
            },
        )]);
    ...
    let state = Arc::new(AppState {
        ...
        clock: Arc::new(clep_api::api::SystemClock),
        vault,
        index: index_handle,
        cas,
        rubbish,
        ...
        feed_runtime: Some(std::sync::Arc::new(
            clep_api::feeds::runtime::FeedRuntime::open(&root, &clep_api::FeedsSettings::default())
                .unwrap(),
        )),
...
async fn transaction_recovery_finalization_failure_reports_all_observed_retained_workspaces() {
    ...
    let error = clep_api::build_app_state(&root)
        .await
        .err()
