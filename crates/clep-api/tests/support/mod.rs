...
use clep_api::api::{AppState, Clock, SystemClock, api_router};
use clep_api::vault::Vault;
use clep_api::vault::academic_hook::AcademicMoveHook;
use clep_api::vault::cas::ContentStore;
use clep_api::vault::hooks::{PostDeleteHook, PostMoveHook};
...
impl ApiFixtureBuilder {
    ...
    pub fn build(self) -> ApiFixture {
        ...
        let archive_resource_concurrency = clep_api::api::archive::archive_resource_concurrency(
            vault.config().archive.max_blob_size_mb,
        );
        let db_path = vault.root().join(".clepsydra/cache.db");
        let mut index = VaultIndex::open(&db_path)
            .unwrap()
            .with_linkable_properties(Box::new(clep_api::vault::base::BaseLinkableProperties));
        ...
        let rubbish = clep_api::vault::rubbish::RubbishStore::for_vault(vault.root());
        let delete_hooks = self
            .delete_hook_factory
            .map_or_else(Vec::new, |factory| factory(&cas));
        let purge_hooks: Arc<Vec<Box<dyn clep_api::vault::hooks::RubbishPurgeHook>>> = Arc::new(
            vec![Box::new(clep_api::vault::archive_hook::ArchiveDeleteHook {
                cas: Arc::clone(&cas),
            })],
        );
        ...
        let state = Arc::new(AppState {
            ...
            features: clep_api::FeatureFlags::default(),
            clock: self.clock,
            vault,
            index,
            cas,
            ...
            purge_hooks,
            mutation_coordinator: clep_api::vault::mutation_coordinator::MutationCoordinator::new(),
            sync: None,
            watcher_paused: Arc::new(std::sync::atomic::AtomicBool::new(false)),
            feed_runtime: Some(Arc::new(
                clep_api::feeds::runtime::FeedRuntime::open(
                    &root,
                    &clep_api::FeedsSettings::default(),
                )
                .unwrap(),
            )),
        ...
        let app: Router = Router::new()
            ...
            .merge(clep_api::api::deeplink::root_router())
