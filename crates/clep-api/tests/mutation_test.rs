...
use clep_api::vault::atomic_file::{
    AtomicPublicationError, atomic_create, atomic_create_owner_only, atomic_replace,
    atomic_replace_with,
};
use clep_api::vault::batch_mutation::{BatchMutationCommand, BatchPathIntent, ExpectedPathState};
...
use clep_api::vault::page::{PageMeta, parse_or_repair_frontmatter};
use clep_api::vault::path::VaultPath;
use clep_api::vault::rubbish::{RubbishListEntry, RubbishManifest, RubbishStore};
use clep_api::vault::sync::ChangeEvent;
...
async fn purge_rubbish_requires_an_opaque_uuid_and_removes_only_that_item_and_catalog_row() {
    ...
    let purge_hooks = Arc::new(vec![
        Box::new(clep_api::vault::archive_hook::ArchiveDeleteHook {
            cas: Arc::clone(&cas),
        }) as Box<dyn clep_api::vault::hooks::RubbishPurgeHook>,
    ]);
...
impl clep_api::vault::hooks::RubbishPurgeHook for RecordingPurgeHook {
    fn on_rubbish_purge(
        &self,
        item_id: Uuid,
...
impl clep_api::vault::hooks::RubbishPurgeHook for SharedHook {
    fn on_rubbish_purge(
        &self,
        i: Uuid,
        p: &VaultPath,
...
async fn purge_invokes_registered_hook_with_item_identity() {
    ...
    let hooks: Arc<Vec<Box<dyn clep_api::vault::hooks::RubbishPurgeHook>>> =
...
fn compute_relative_path_same_dir() {
    use clep_api::vault::mutation::compute_relative_path;
