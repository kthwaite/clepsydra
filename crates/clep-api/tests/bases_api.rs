...
async fn create_base_member_writes_one_matching_typed_page() {
    ...
    let vault_path = clep_api::vault::path::VaultPath::new(path).unwrap();
    let page = clep_api::vault::page::Page::from_file(
    ...
    assert_eq!(
        body["revision"],
        clep_api::vault::page::page_revision(&page.raw_content)
    );
    assert_eq!(page.meta.kind, Some(clep_api::vault::kind::Kind::Book));
...
async fn member_creation_coerces_every_custom_property_type_and_compound_filters() {
    ...
    response.assert_status(StatusCode::CREATED);
    let body: serde_json::Value = response.json();
    let path = clep_api::vault::path::VaultPath::new(body["path"].as_str().unwrap()).unwrap();
    let page =
        clep_api::vault::page::Page::from_file(&fixture.state.vault.resolve(&path), path).unwrap();
    assert_eq!(
        page.meta.kind,
        Some(clep_api::vault::kind::Kind::Note),
...
async fn cancelled_member_request_still_emits_one_event_after_commit() {
    ...
    fixture
        ...
        .set_create_publication_hook(Some(Arc::new({
            ...
            move |path, content| {
                ...
                clep_api::vault::atomic_file::atomic_create(path, content)
            }
        })));
    let mut notifications = fixture.state.change_tx.subscribe();
    ...
    let created_path = clep_api::vault::path::VaultPath::new(&upserted[0]).unwrap();
